;; taken from https://github.com/metosin/malli/pull/211 with some additions
...
(defn properties->malli [required [k v]]
  (println required [k v])
  (cond-> [k]
    (:required v) (conj {:required true})
    (nil? (:required v)) (conj {:optional true})
...
(defmethod type->malli "integer" [{:keys [minimum maximum ;;exclusiveMinimum exclusiveMaximum multipleOf
                                   ...
                                   :or {minimum #?(:clj Integer/MIN_VALUE :cljs js/Number.MIN_SAFE_INTEGER)
                                        maximum #?(:clj Integer/MAX_VALUE :cljs js/Number.MAX_SAFE_INTEGER)}}]
...
(defmethod type->malli "number" [_] number?)
(defmethod type->malli "boolean" [_] boolean?)
(defmethod type->malli "null" [_] nil?)
