...
export const loader = async ({ request }) => {
  ...
  if (!signature) {
    return redirect(`/proof?address=${address}${invite ? `&invite=${invite}` : ''}`)
  }

  //@ts-ignore
  const reservation = await RESERVE.get('reservation', { type: 'json' })
  if (!reservation || reservation.address == address) {
    return redirect(`/redeem?address=${address}&signature=${signature}${invite ? `&invite=${invite}` : ''}`)
  }
    ...
    <div className="mx-auto text-center">
      ...
      <div className="row d-flex align-self-center">
        <div className="col-12 mx-auto text-center">
          ...
          >
            <Countdown date={Date.now() + 15000} renderer={reserveTimerRender} />          
          </button>
        </div>
      </div>
    </div>
