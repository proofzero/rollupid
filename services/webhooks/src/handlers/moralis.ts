...
const MoralisHandler = async (request: Request, env: Environment) => {
  ...
  const digestText = new TextEncoder().encode(bodyText + env.APIKEY_MORALIS)
  const digest = await crypto.subtle.digest('SHA-256', digestText)
  const digestArray = Array.from(new Uint8Array(digest))
  const generatedSignature = digestArray
