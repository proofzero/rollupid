...
describe("Invitations", function () {
  ...
  describe("Signatures", function () {
    it("Should validate operator signatures", async function () {
      ...
      expect(await invite._recoverVoucherSigner(voucher)).to.equal(operator.address);
    });
  });
  ...
  describe("Invites", function () {
    it("Should reject out-of-order invites", async function () {
      const { invite, voucher, owner, operator } = await loadFixture(deployInviteFixture);
      const nextVoucher = await signVoucher(owner, 'https://example.com', 5, operator);
      await expect(invite.awardInvite(owner.address, nextVoucher)).to.be.rejected;
    });

    it("Should reject bad vouchers", async function () {
      const { invite, voucher, owner, operator } = await loadFixture(deployInviteFixture);
      const nextVoucher = await signVoucher(owner, 'https://example.com', 5, owner);
      await expect(invite.awardInvite(owner.address, nextVoucher)).to.be.rejected;
    });

    it("Should reject bad recipients", async function () {
      const { invite, voucher, owner, operator } = await loadFixture(deployInviteFixture);
      const nextVoucher = await signVoucher(owner, 'https://example.com', 1, operator);
      await expect(invite.awardInvite(operator.address, nextVoucher)).to.be.rejected;
    });
