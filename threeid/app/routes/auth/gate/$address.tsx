...
export const loader = async ({ request, params }) => {
  ...
  if (session.has("jwt")) {
    const inviteRes = await oortSend(
      "3id_listInvitations",
      [],
...
export async function action({ request, params }) {
  ...
  const redeemRes = await oortSend(
    "3id_redeemInvitation",
    [formData.get("contractAddress"), formData.get("tokenId")],
    params.address,
    session.get("jwt"),
    request.headers.get("Cookie")
    ...
    return (
      <div className="gate justify-center items-center">
        ...
        <p className="auth-secondary-message">
          Select an invite to activate your 3ID.
        </p>
              ...
              <>
                ...
                <div className="grid grid-rows-2 lg:grid-cols-2">
                  ...
                  <BaseButton
                    ...
                    color={"dark"}
                    onClick={() => submit(selected, { method: "post" })}
                  />
                </div>
              </>
            )}
