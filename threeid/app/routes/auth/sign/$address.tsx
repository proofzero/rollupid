...
export async function action({ request, params }) {
  ...
  const signRes = await oortSend(
    "kb_verifyNonce",
    [formData.get("nonce"), formData.get("signature")],
    params.address
...
export default function AuthSign() {
  ...
  const { data, error, isLoading, signMessage } = useSignMessage({
    onSuccess(data, variables) {
      submit(
        { signature: data, nonce: sign.nonce },
        { method: "post", action: `/auth/sign/${address}` }
      );
    },
  });
  ...
  return (
    <div className="justify-center items-center">
      <p className="auth-message">Please sign the auth message.</p>
