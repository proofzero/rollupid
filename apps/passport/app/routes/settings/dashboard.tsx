  ...
  return (
    <>
      ...
      <div className="w-full h-full flex flex-col">
        <div className="pb-6 flex flex-row items-center gap-4">
          <Text weight="semibold" size="2xl">
            Welcome to Passport
          </Text>

          <button
            className="pl-2 pr-4 py-2 rounded-full bg-white flex items-center gap-2 hover:bg-gray-100"
            onClick={() => setEditProfileModalOpen(true)}
          >
            <img src={pfpUrl} className="w-8 h-8 rounded-full" />
            <Text size="lg" weight="semibold" className="text-gray-800">
              {displayName}
            </Text>
            <TbPencil className="w-5 h-5 text-gray-500" />
          </button>
        </div>
        {appErrorExists ? (
          <WarningCTA
