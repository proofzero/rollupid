...
const Dropdown = (props: DropdownProps) => {
  ...
  switch (props.position) {
    case DropdownPosition.START:
      ...
      buttonClasses = ['border-r-1', 'rounded-r-none'].join(' ')
      break
    case DropdownPosition.MIDDLE:
      break
    case DropdownPosition.END:
      buttonClasses = ['border-l-0', 'rounded-l-none'].join(' ')
      break
  }
  ...
  return (
    <div className="relative inline-block text-left">
      <div>
        <button
          type="button"
          className={`inline-flex w-full justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-100 ${buttonClasses}`}
          id="menu-button"
          aria-expanded="true"
          aria-haspopup="true"
        >
      ...
      <label>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-row">
            <Dropdown
              position={DropdownPosition.START}
              label="Profile"
              items={['foo', 'bar', 'baz']}
            />
            <Dropdown
              position={DropdownPosition.END}
              label="All Data"
              items={['foo', 'bar', 'baz']}
            />
          </div>
