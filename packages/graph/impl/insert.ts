...
import type { EdgeId, EdgeTag, Graph } from '../types'
 ...
 */
export async function node(g: Graph, urn: AnyURN) {
  return new Promise(async (resolve, reject) => {
    ...
    // prepared statements.
    const compResult = await g.db.batch(stmts)
    ...
    const nodeResult = await g.db.prepare(
